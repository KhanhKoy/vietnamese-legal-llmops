...
def show():
    ...
    tab_titles = ["📊 Dashboard", "👥 Quản Lý Người Dùng", "📜 Logs & Lịch Sử", "⚙️ Cài Đặt Hệ Thống"]
    ...
    users = list_users()
    st.session_state.users = {user["username"]: user for user in users}

    with tab_dashboard:
        st.subheader("📊 Chỉ Số Tổng Quan (KPI Cards)")
        col_m1, col_m2, col_m3, col_m4, col_m5, col_m6 = st.columns(6)
        col_m1.metric("Tổng User", f"{stats['user_count']}", "+12 tuần này")
        col_m2.metric("Tổng Câu Hỏi", f"{stats['session_count']}", "+850 hôm nay")
        col_m3.metric("Tỷ Lệ Hài Lòng", f"{stats['feedback_count']} feedback", "+1.5%")
        col_m4.metric("User Active", f"{stats['active_count']}", "+24%")
        col_m5.metric("Tổng Session", f"{stats['session_count']}", "+350K")
        col_m6.metric("Feedback", f"{stats['feedback_count']}", "+$1.20")
            ...
            [
                {
                    ...
                    "Đánh giá": item["feedback"],
                }
                for idx, item in enumerate(get_recent_activity(10), start=1)
            ]
        )
    ...
    with tab_users:
        ...
        with col_s4:
            ...
            with st.popover("➕ Thêm User Mới"):
                ...
                if st.button("Lưu Người Dùng Mới", type="primary"):
                    ...
                    elif any(user["username"] == u_clean for user in users):
        ...
        for data in users:
            ...
            if status_filter != "Tất cả" and u_status != status_filter:
                continue
            if search_kw and (search_kw.lower() not in username.lower() and search_kw.lower() not in data["email"].lower()):
                continue
        ...
        target_usr = col_act1.selectbox("Chọn User thao tác", [u["username"] for u in users])

        target_data = next((u for u in users if u["username"] == target_usr), {})
        ...
        with col_act2:
            if not is_inactive:
                ...
                if st.button("🟢 Khôi phục tài khoản", type="primary"):
                    update_user_status(target_usr, "Active", False, None)
    ...
    with tab_logs:
        ...
        logs_df = pd.DataFrame(
            [
                ...
                for idx, item in enumerate(activity, start=1)
            ]
        )
    ...
    with tab_settings:
        ...
        with col_cfg2:
            ...
            st.write("- **Loại DB:** SQLite (sẵn sàng chuyển sang PostgreSQL/RDS)")
